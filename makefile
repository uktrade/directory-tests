...
SET_DB_URLS := \
	export DIR_DATABASE_URL=`/usr/bin/heroku config:get DATABASE_URL -a directory-api-dev` && \
	export SSO_DATABASE_URL=`/usr/bin/heroku config:get DATABASE_URL -a directory-sso-dev`

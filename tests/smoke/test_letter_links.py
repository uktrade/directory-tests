...
def test_access_as_anon_user(absolute_url, exred_hawk_cookie):
    response = requests.get(absolute_url, allow_redirects=True, cookies=exred_hawk_cookie)
...
def test_access_contact_us_as_anon_user_after_removing_trailing_slash(
        absolute_url, exred_hawk_cookie):
...
def test_302_redirects_for_anon_user(
        absolute_url, exred_hawk_cookie):
    response = requests.get(absolute_url, allow_redirects=False, cookies=exred_hawk_cookie)
...
def test_access_endpoints_as_logged_in_user(
        logged_in_session, absolute_url, exred_hawk_cookie):
    response = logged_in_session.get(absolute_url, allow_redirects=True, cookies=exred_hawk_cookie)
    assert response.status_code == http.client.OK

...
def test_access_as_anon_user(absolute_url, basic_auth, hawk_cookie):
    response = requests.get(absolute_url, allow_redirects=True, auth=basic_auth, cookies=hawk_cookie)
...
def test_302_redirects_for_anon_user(
        absolute_url, basic_auth, hawk_cookie):
    response = requests.get(absolute_url, allow_redirects=False, auth=basic_auth, cookies=hawk_cookie)
...
def test_access_endpoints_as_logged_in_user(
    ...
    response = logged_in_session.get(absolute_url, allow_redirects=True, auth=basic_auth, cookies=hawk_cookie)
    assert response.status_code == http.client.OK

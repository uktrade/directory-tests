...
def test_landing_page_post_company_not_active(basic_auth, hawk_cookie):
...
def test_landing_page_post_company_already_registered(basic_auth, hawk_cookie):
...
def test_landing_page_post_company_not_found(basic_auth, hawk_cookie):
...
def test_landing_page_post_company_happy_path(basic_auth, hawk_cookie):
...
def test_not_existing_page_return_404_anon_user(basic_auth, hawk_cookie):
    url = get_absolute_url("ui-buyer:landing") + "/foobar"
    response = requests.get(url, allow_redirects=False, auth=basic_auth, cookies=hawk_cookie)
...
def test_not_existing_page_return_404_user(logged_in_session, basic_auth, hawk_cookie):
    ...
    response = logged_in_session.get(
        url, allow_redirects=False, auth=basic_auth, cookies=hawk_cookie
    )
    assert response.status_code == 404

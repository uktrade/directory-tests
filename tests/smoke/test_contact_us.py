...
def test_redirects_for_legacy_contact_us_urls(endpoint, expected_redirect, basic_auth):
    ...
    if response.history:
        ...
        error_message = (
            f"Expected {url} redirect to: '{expected_redirect}' "
...
@pytest.mark.parametrize(
    ...
    [
        (
            urljoin(URLs.LEGACY_CONTACT_US_LANDING.absolute, "soo/TriageForm/"),
            URLs.SSO_LOGIN.absolute,
        ),
        (
            urljoin(URLs.LEGACY_CONTACT_US_LANDING.absolute, "triage/soo/"),
            URLs.SSO_LOGIN.absolute,
        ),
    ],
)
def test_legacy_contact_us_soo_urls_should_redirect_to_sso(
    ...
    error = (
        f"After logging in user should be redirected to "
...
@pytest.mark.parametrize(
    "endpoint, expected_redirect", [("directory/FeedbackForm/", "/contact/feedback/")]
)

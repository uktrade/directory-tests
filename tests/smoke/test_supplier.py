...
def test_landing_page_200(hawk_cookie):
    response = requests.get(
        get_absolute_url('ui-supplier:landing'), cookies=hawk_cookie
    )

    assert response.status_code == http.client.OK


def test_supplier_list_200(hawk_cookie):
    response = requests.get(
        get_absolute_url('ui-supplier:suppliers'), cookies=hawk_cookie
    )

    assert response.status_code == http.client.OK


def test_industries_list_200(hawk_cookie):
    response = requests.get(
        get_absolute_url('ui-supplier:industries'), cookies=hawk_cookie
    )

    assert response.status_code == http.client.OK


def test_health_industry_200(hawk_cookie):
    response = requests.get(
...
def test_tech_industry_200(hawk_cookie):
    response = requests.get(
...
def test_creative_industry_200(hawk_cookie):
    url = get_absolute_url('ui-supplier:industries-creative')
    response = requests.get(url, cookies=hawk_cookie)

    assert response.status_code == http.client.OK


def test_food_industry_200(hawk_cookie):
    response = requests.get(
...
def test_supplier_profile_200(hawk_cookie):
...
def test_supplier_contact_200(hawk_cookie):
    # company 09466005 must exist on the environment the tests are ran against.
...
def test_case_study_200(hawk_cookie):

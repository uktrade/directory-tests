...
CASE_STUDIES = {
    1: {
        "breadcrumb": Selector(By.CSS_SELECTOR, "#content > div > p.breadcrumbs > span.current"),
        ...
        if title:
            if element_name in ["title", "breadcrumb"]:
                element = find_element(
                    driver, selector, element_name=element_name
                )
                with assertion_msg(
...
def should_see_share_widget(driver: WebDriver):
    share_widget = find_element(
        driver, SHARE_WIDGET, element_name="Share widget"
    )

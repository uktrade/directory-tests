...
REGISTER = Selector(By.CSS_SELECTOR, "#articles > div > div.scope-indicator > p > a:nth-child(1)")
SIGN_IN = Selector(By.CSS_SELECTOR, "#articles > div > div.scope-indicator > p > a:nth-child(2)")
...
def show_more(driver: WebDriver):
    button = find_element(
        driver, SHOW_MORE_BUTTON, element_name="Show more button"
    )
...
def show_all_articles(driver: WebDriver):
    ...
    try:
        show_more_button = find_element(
            driver, SHOW_MORE_BUTTON, wait_for_it=False
        )
        max_clicks = 10
        counter = 0
